...
#define STRING_CONFIG_H_AUTHOR "(@yonhun)" // Who made the changes.
...
#define CUSTOM_MACHINE_NAME "PARALLEL SCARA"
...
// 여기에는 만약에 서미스터 타입이 다를 경우 1이 아닌 다른 서미스터 타입을 기입해줘야함(핫엔드 한개 + 핫베드 한개)
...
//RESIDENCY_TIME(잔류 시간): 목표 온도에 도달한 후 얼마나 안정적으로 유지되는지 확인하는 시간 -> 확인 후에 출력을 시작함
//WINDOW(온도 창): 목표 온도로부터 허용되는 변동 범위
//HYSTERESIS(히스테리시스): 히터 켜짐/꺼짐 전환 시 허용되는 온도 차이
//if target = 25°C일 때 window = 1°C면 24°C~26°C일 경우에는 장치가 켜지거나 꺼지지 않음.
//if target = 25°C일 때 hysteresis = 3°C면 22°C 이하로 내려가면 히터가 켜지고 28°C 이상이면 히터가 꺼짐 
...
//PID_FUNCTIONAL_RANGE같은 경우 작을 경우 초기 가열이 빠른 대신 덜 정밀한 제어, 클 경우 초기 가열이 느린 대신 더 정밀한 제어 가능
...
#define PARALLEL_SCARA
  ...
  #define SCARA_LINKAGE_1 90        // (mm)
  #define SCARA_LINKAGE_2 113       // (mm)
 ...
 */
#define DEFAULT_AXIS_STEPS_PER_UNIT   { 400/9, 400/9, 200, 114 }
